...
`:target`当URL中的哈希值和元素的ID相同时，请使用伪类为元素设置样式。查看[此演示](https://jec.fyi/demo/css-target#section-2)，自己尝试。借助新的DevTools功能，您可以测试此类样式，而不必始终手动更改URL。
...
video 标签动态切换src的时候, 会有缓存需要重新创建 video 标签.
